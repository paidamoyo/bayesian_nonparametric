function [pre_rate, eval_result, model_test,y,accuracy,accuracy1] = sLDA_test(testdata, model, vbe_max_iter);
...
for i=1:testdata.docnum,
    ...
    aver_beta = sum(diag(testdata.doc(i).word)*model_test.phi(testdata.doc(i).word_id, :),1)...
...
for i=1:testdata.docnum
    if pre_rate(i)<1
        y(3,i)=1;
    else if pre_rate(i)>20
            y(3,i)=20;
        else if (pre_rate(i)-fix(pre_rate(i))<0.5)
                y(3,i)=fix(pre_rate(i));
            else
                y(3,i)=fix(pre_rate(i))+1;
            end
        end
    end
end
number=0;
for i=1:testdata.docnum
    if y(1,i)-2<y(3,i)&&y(1,i)+2>y(3,i)
        number=number+1;
    end
end
number1=0;
...
accuracy=number/testdata.docum;
accuracy1=number1/testdata.docum;
